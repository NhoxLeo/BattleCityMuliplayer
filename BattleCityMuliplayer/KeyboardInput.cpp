...
KeyboardInput* KeyboardInput::m_pDxInput = nullptr;

KeyboardInput* KeyboardInput::GetInstance()
{
	if (m_pDxInput == nullptr)
		m_pDxInput = new KeyboardInput();
...
HRESULT KeyboardInput::Create(HWND _hwnd)
{
	m_hWnd = _hwnd;
...
bool KeyboardInput::Keyboard_Up(const int key)
{
	if (!INPUTEVENT(m_key_buffer, key) && m_key_down[key])
		return true;
	return false;
}
bool KeyboardInput::Keyboard_Down(const int key)
{
	if (INPUTEVENT(m_key_buffer, key) && !m_key_down[key])
		return true;
	return false;
}
bool KeyboardInput::Keyboard_DownState(const int key)
{
	if (INPUTEVENT(m_key_buffer, key))
		return true;
	return false;
}
bool KeyboardInput::Mouse_Up(const MOUSE_STATE n)
{
	if (!INPUTEVENT(m_MouseState.rgbButtons, n) && m_button_down[n])
		return true;
	return false;
}
bool KeyboardInput::Mouse_Down(const MOUSE_STATE n)
{
	if (INPUTEVENT(m_MouseState.rgbButtons, n) && !m_button_down[n])
		return true;
	return false;
}
bool KeyboardInput::Mouse_DownState(const MOUSE_STATE n)
{
...
bool KeyboardInput::Mouse_DoubleClick(const MOUSE_STATE n, float _timing)
{
